...
PNG ──Decode──► want *image.NRGBA
                    │
                    ▼
              Search(ctx, want) ──► Epoch{Document, Scale, Operator, Elapsed} ── Last ──► Encode──► XML
                    │
...
type Epoch struct {
    ...
    Island   *image.NRGBA  // leftover mask: Score miss (white) and inscribed triangle (orange); nil if unpublished
}
