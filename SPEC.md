...
Search has autonomy over palette, Loss, and mutate. The CLI does not inject a ColorMap or a color count. `--search` selects the registry name. Default `dumb`. `simplify` grows the palette: `palette.Auto(k)` for k=1,2,… and keeps the last k whose painted islands lower Fit. Stops after two k in a row that do not. Then peel a layer when Fit improves. Fill is the mean of the island’s PNG pixels. A round closed loop (radius within 20% of the mean) is two cubics, not an octagon of lines. Other sharp turns ≥40° with legs ≥4px stay. Tiny ears may shave a 1px stair. RDP then runs per span. Cubic slop grows until the command count stalls.
